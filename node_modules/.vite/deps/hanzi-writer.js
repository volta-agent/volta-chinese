...
function copyAndMergeDeep(base, override) {
	...
	for (const key in override) {
		...
		if (baseVal === overrideVal) continue;
		if (baseVal && overrideVal && typeof baseVal === "object" && typeof overrideVal === "object" && !Array.isArray(overrideVal)) output[key] = copyAndMergeDeep(baseVal, overrideVal);
		else output[key] = overrideVal;
	}
	return output;
}
...
function average(arr) {
	return arr.reduce((acc, val) => val + acc, 0) / arr.length;
}
...
function objRepeat(item, times) {
	const obj = {};
	for (let i = 0; i < times; i++) obj[i] = item;
	return obj;
}
function objRepeatCb(times, cb) {
	const obj = {};
	for (let i = 0; i < times; i++) obj[i] = cb(i);
	return obj;
}
...
var RenderState = class {
	...
	_run(mutationChain) {
		...
		if (mutationChain._index >= mutations.length) if (mutationChain._loop) mutationChain._index = 0;
		else {
		...
		mutationChain._mutations[mutationChain._index].run(this).then(() => {
	...
	cancelMutations(scopesToCancel) {
		for (const chain of this._mutationChains) for (const chainId of chain._scopes) for (const scopeToCancel of scopesToCancel) if (chainId.startsWith(scopeToCancel) || scopeToCancel.startsWith(chainId)) this._cancelMutationChain(chain);
	}
	cancelAll() {
	...
	_cancelMutationChain(mutationChain) {
		...
		mutationChain._isActive = false;
		for (let i = mutationChain._index; i < mutationChain._mutations.length; i++) mutationChain._mutations[i].cancel(this);
		(_mutationChain$_resol = mutationChain._resolve) === null || _mutationChain$_resol === void 0 || _mutationChain$_resol.call(mutationChain, { canceled: true });
		this._mutationChains = this._mutationChains.filter((chain) => chain !== mutationChain);
	}
};
var subtract = (p1, p2) => ({
...
var frechetDist = (curve1, curve2) => {
	...
	const calcVal = (i, j, prevResultsCol, curResultsCol) => {
		...
		if (i > 0 && j === 0) return Math.max(prevResultsCol[0], distance(longCurve[i], shortCurve[0]));
		const lastResult = curResultsCol[curResultsCol.length - 1];
		if (i === 0 && j > 0) return Math.max(lastResult, distance(longCurve[0], shortCurve[j]));
	...
	for (let i = 0; i < longCurve.length; i++) {
		const curResultsCol = [];
		for (let j = 0; j < shortCurve.length; j++) curResultsCol.push(calcVal(i, j, prevResultsCol, curResultsCol));
...
var subdivideCurve = (curve, maxLen = .05) => {
	...
	for (const point of curve.slice(1)) {
		...
		if (segLen > maxLen) {
			...
			for (let i = 0; i < numNewPoints; i++) newCurve.push(_extendPointOnLine(point, prevPoint, -1 * newSegLen * (i + 1)));
		} else newCurve.push(point);
	}
	return newCurve;
};
...
var normalizeCurve = (curve) => {
	...
	return subdivideCurve(translatedCurve.map((point) => ({
		x: point.x / scale,
		y: point.y / scale
	})));
};
...
var _filterParallelPoints = (points) => {
	...
	points.slice(2).forEach((point) => {
		...
		if (curVect.y * prevVect.x - curVect.x * prevVect.y === 0) filteredPoints.pop();
...
function generateStrokes({ radStrokes, strokes, medians }) {
	...
	return strokes.map((path, index) => {
		return new Stroke(path, medians[index].map((pointData) => {
			const [x, y] = pointData;
			return {
				x,
				y
			};
		}), index, isInRadical(index));
	});
}
function parseCharData(symbol, charJson) {
	return new Character(symbol, generateStrokes(charJson));
}
var [from, to] = [{
	x: 0,
	y: -124
...
var FRECHET_THRESHOLD = .4;
var MIN_LEN_THRESHOLD = .35;
function strokeMatches(userStroke, character, strokeNum, options = {}) {
	...
	for (let i = 0; i < laterStrokes.length; i++) {
		...
		if (isMatch && avgDist < closestMatchDist) closestMatchDist = avgDist;
	}
	if (closestMatchDist < avgDist) {
		const leniencyAdjustment = .6 * (closestMatchDist + avgDist) / (2 * avgDist);
...
var directionMatches = (points, stroke) => {
	...
	return average(edgeVectors.map((edgeVector) => {
...
var stripDuplicates = (points) => {
	...
	for (const point of rest) if (!equals(point, dedupedPoints[dedupedPoints.length - 1])) dedupedPoints.push(point);
	return dedupedPoints;
};
var SHAPE_FIT_ROTATIONS = [
	Math.PI / 16,
	Math.PI / 32,
	0,
	-1 * Math.PI / 32,
...
var getMatchData = (points, stroke, options) => {
	...
	const withinDistThresh = avgDist <= averageDistanceThreshold * (isOutlineVisible || stroke.strokeNum > 0 ? .5 : 1) * leniency;
	if (!withinDistThresh) return {
		isMatch: false,
		avgDist,
		meta: { isStrokeBackwards: false }
	};
	...
	if (checkBackwards && !isMatch) {
		if (getMatchData([...points].reverse(), stroke, {
			...options,
			checkBackwards: false
		}).isMatch) return {
			isMatch,
...
var Mutation = class {
	...
	*/
	...
	run(renderState) {
		...
		if (this._duration === 0 || isAlreadyAtEnd(renderState.state, this._values)) return Promise.resolve();
	...
	_inflateValues(renderState) {
		...
		if (typeof this._valuesOrCallable === "function") values = this._valuesOrCallable(renderState.state);
	...
	cancel(renderState) {
		var _this$_resolve;
		(_this$_resolve = this._resolve) === null || _this$_resolve === void 0 || _this$_resolve.call(this);
		this._resolve = void 0;
...
function getPartialValues(startValues, endValues, progress) {
	...
	for (const key in endValues) {
		...
		if (typeof startValue === "number" && typeof endValue === "number" && endValue >= 0) target[key] = progress * (endValue - startValue) + startValue;
		else target[key] = getPartialValues(startValue, endValue, progress);
	}
	return target;
}
function isAlreadyAtEnd(startValues, endValues) {
	for (const key in endValues) {
		...
		} else if (!isAlreadyAtEnd(startValue, endValue)) return false;
	}
	return true;
}
var ease = (x) => -Math.cos(x * Math.PI) / 2 + .5;
...
var animateSingleStroke = (charName, character, strokeNum, speed) => {
	const mutationStateFunc = (state) => {
		...
		for (let i = 0; i < character.strokes.length; i++) mutationState.strokes[i] = { opacity: curCharState.opacity * curCharState.strokes[i].opacity };
...
var startQuiz = (character, fadeDuration, startStrokeNum) => {
	return [
		...
		new Mutation("character.main", {
			opacity: 1,
			strokes: objRepeatCb(character.strokes.length, (i) => ({ opacity: i < startStrokeNum ? 1 : 0 }))
		}, { force: true })
	];
};
...
var removeAllUserStrokes = (userStrokeIds) => {
	return (userStrokeIds === null || userStrokeIds === void 0 ? void 0 : userStrokeIds.map((userStrokeId) => new Mutation(`userStrokes.${userStrokeId}`, null, { force: true }))) || [];
};
...
var Quiz = class {
	...
	startQuiz(options) {
		if (this._userStrokesIds) this._renderState.run(removeAllUserStrokes(this._userStrokesIds));
	...
	startUserStroke(externalPoint) {
		...
		(_this$_userStrokesIds = this._userStrokesIds) === null || _this$_userStrokesIds === void 0 || _this$_userStrokesIds.push(strokeId);
	...
	endUserStroke() {
		...
		if (isMatch || isForceAccepted || meta.isStrokeBackwards && acceptBackwardsStrokes) this._handleSuccess(meta);
		else {
			this._handleFailure(meta);
			const { showHintAfterMisses, highlightColor, strokeHighlightSpeed } = this._options;
			if (showHintAfterMisses !== false && this._mistakesOnStroke >= showHintAfterMisses) this._renderState.run(highlightStroke(currentStroke, colorStringToVals(highlightColor), strokeHighlightSpeed));
		}
		this._userStroke = void 0;
	}
	cancel() {
	...
	nextStroke() {
		...
		if (this._currentStrokeIndex === strokes.length) {
			this._isActive = false;
			onComplete === null || onComplete === void 0 || onComplete({
				character: symbol,
				totalMistakes: this._totalMistakes
			});
			if (highlightOnComplete) animation = animation.concat(highlightCompleteChar(this._character, colorStringToVals(highlightCompleteColor || highlightColor), (strokeHighlightDuration || 0) * 2));
		}
	...
	_handleSuccess(meta) {
		...
		onCorrectStroke === null || onCorrectStroke === void 0 || onCorrectStroke({ ...this._getStrokeData({
			isCorrect: true,
			meta
		}) });
		this.nextStroke();
	...
	_handleFailure(meta) {
		...
		(_this$_options$onMist = (_this$_options = this._options).onMistake) === null || _this$_options$onMist === void 0 || _this$_options$onMist.call(_this$_options, this._getStrokeData({
			isCorrect: false,
			meta
		}));
	}
...
function urlIdRef(id) {
	let prefix = "";
	if (window.location && window.location.href) prefix = window.location.href.replace(/#[^#]*$/, "").replace(/"/gi, "%22");
	return `url("${prefix}#${id}")`;
}
function removeElm(elm) {
	var _elm$parentNode;
	elm === null || elm === void 0 || (_elm$parentNode = elm.parentNode) == null || _elm$parentNode.removeChild(elm);
}
var StrokeRendererBase = class StrokeRendererBase {
	constructor(stroke) {
		this.stroke = stroke;
		this._pathLength = stroke.getLength() + StrokeRendererBase.STROKE_WIDTH / 2;
	}
	_getStrokeDashoffset(displayPortion) {
		return this._pathLength * .999 * (1 - displayPortion);
	}
...
var StrokeRenderer = class extends StrokeRendererBase {
	...
	render(props) {
		...
		if (props === this._oldProps || !this._animationPath) return;
		if (props.displayPortion !== ((_this$_oldProps = this._oldProps) === null || _this$_oldProps === void 0 ? void 0 : _this$_oldProps.displayPortion)) this._animationPath.style.strokeDashoffset = this._getStrokeDashoffset(props.displayPortion).toString();
		const color = this._getColor(props);
		if (!this._oldProps || color !== this._getColor(this._oldProps)) {
			const { r, g, b, a } = color;
			attrs(this._animationPath, { stroke: `rgba(${r},${g},${b},${a})` });
		}
		if (props.opacity !== ((_this$_oldProps2 = this._oldProps) === null || _this$_oldProps2 === void 0 ? void 0 : _this$_oldProps2.opacity)) this._animationPath.style.opacity = props.opacity.toString();
...
var CharacterRenderer = class {
	...
	render(props) {
		...
		if (props === this._oldProps || !this._group) return;
		...
		if (opacity !== ((_this$_oldProps = this._oldProps) === null || _this$_oldProps === void 0 ? void 0 : _this$_oldProps.opacity)) {
			...
			if (!isMsBrowser) {
				...
				if (opacity === 0) this._group.style.display = "none";
				else if (((_this$_oldProps2 = this._oldProps) === null || _this$_oldProps2 === void 0 ? void 0 : _this$_oldProps2.opacity) === 0) this._group.style.removeProperty("display");
			}
		}
		const colorsChanged = !this._oldProps || strokeColor !== this._oldProps.strokeColor || radicalColor !== this._oldProps.radicalColor;
		if (colorsChanged || strokes !== ((_this$_oldProps3 = this._oldProps) === null || _this$_oldProps3 === void 0 ? void 0 : _this$_oldProps3.strokes)) for (let i = 0; i < this._strokeRenderers.length; i++) {
			var _this$_oldProps4;
			if (!colorsChanged && (_this$_oldProps4 = this._oldProps) !== null && _this$_oldProps4 !== void 0 && _this$_oldProps4.strokes && strokes[i] === this._oldProps.strokes[i]) continue;
...
var UserStrokeRenderer = class {
	...
	render(props) {
		...
		if (!this._path || props === this._oldProps) return;
		...
		if (props.opacity !== ((_this$_oldProps3 = this._oldProps) === null || _this$_oldProps3 === void 0 ? void 0 : _this$_oldProps3.opacity)) attr(this._path, "opacity", props.opacity.toString());
		if (props.points !== ((_this$_oldProps4 = this._oldProps) === null || _this$_oldProps4 === void 0 ? void 0 : _this$_oldProps4.points)) attr(this._path, "d", getPathString(props.points));
		this._oldProps = props;
	}
	destroy() {
...
var HanziWriterRenderer = class {
	...
	render(props) {
		...
		for (const userStrokeId in this._userStrokeRenderers) if (!userStrokes[userStrokeId]) {
			var _this$_userStrokeRend;
			(_this$_userStrokeRend = this._userStrokeRenderers[userStrokeId]) === null || _this$_userStrokeRend === void 0 || _this$_userStrokeRend.destroy();
		...
		for (const userStrokeId in userStrokes) {
			...
			(() => {
				if (this._userStrokeRenderers[userStrokeId]) return this._userStrokeRenderers[userStrokeId];
...
var RenderTargetBase = class {
	...
	_getTouchPoint(evt) {
		...
		return {
			...
			y: evt.touches[0].clientY - top
		};
	}
};
var svgRenderer = {
	HanziWriterRenderer,
	createRenderTarget: class RenderTarget extends RenderTargetBase {
		constructor(svg, defs) {
			super(svg);
			this.svg = svg;
			this.defs = defs;
			if ("createSVGPoint" in svg) this._pt = svg.createSVGPoint();
		}
		static init(elmOrId, width = "100%", height = "100%") {
			const element = (() => {
				if (typeof elmOrId === "string") return document.getElementById(elmOrId);
				return elmOrId;
			})();
			if (!element) throw new Error(`HanziWriter target element not found: ${elmOrId}`);
			const nodeType = element.nodeName.toUpperCase();
			const svg = (() => {
				if (nodeType === "SVG" || nodeType === "G") return element;
				else {
...
var pathStringToCanvas = (pathString) => {
	...
	for (const part of pathParts) {
		...
		if (cmd === "M") commands.push((ctx) => ctx.moveTo(...params));
		else if (cmd === "L") commands.push((ctx) => ctx.lineTo(...params));
		else if (cmd === "C") commands.push((ctx) => ctx.bezierCurveTo(...params));
		else if (cmd === "Q") commands.push((ctx) => ctx.quadraticCurveTo(...params));
	}
...
var StrokeRenderer$1 = class extends StrokeRendererBase {
	constructor(stroke, usePath2D = true) {
		super(stroke);
		if (usePath2D && Path2D) this._path2D = new Path2D(this.stroke.path);
		else this._pathCmd = pathStringToCanvas(this.stroke.path);
		this._extendedMaskPoints = extendStart(this.stroke.points, StrokeRendererBase.STROKE_WIDTH / 2);
	}
	render(ctx, props) {
		if (props.opacity < .05) return;
		ctx.save();
		if (this._path2D) ctx.clip(this._path2D);
		else {
			var _this$_pathCmd;
			(_this$_pathCmd = this._pathCmd) === null || _this$_pathCmd === void 0 || _this$_pathCmd.call(this, ctx);
			ctx.globalAlpha = 0;
			ctx.stroke();
			ctx.clip();
		}
...
var CharacterRenderer$1 = class {
	...
	render(ctx, props) {
		...
		for (let i = 0; i < this._strokeRenderers.length; i++) this._strokeRenderers[i].render(ctx, {
			strokeColor,
			radicalColor,
	...
	render(props) {
		...
		this._animationFrame((ctx) => {
			...
			for (const userStrokeId in userStrokes) {
				...
				if (userStroke) renderUserStroke(ctx, {
					strokeWidth: drawingWidth,
					strokeColor: drawingColor,
					...userStroke
				});
...
var canvasRenderer = {
	...
	createRenderTarget: class RenderTarget$1 extends RenderTargetBase {
		constructor(canvas) {
			super(canvas);
		}
		static init(elmOrId, width = "100%", height = "100%") {
			...
			const nodeType = element.nodeName.toUpperCase();
			const canvas = (() => {
				if (nodeType === "CANVAS") return element;
...
var defaultCharDataLoader = (char, onLoad, onError) => {
	...
	xhr.onreadystatechange = () => {
		...
		if (xhr.status === 200) onLoad(JSON.parse(xhr.responseText));
		else if (xhr.status !== 0 && onError) onError(xhr);
	};
	xhr.send(null);
};
var defaultOptions = {
...
var LoadingManager = class {
	...
	_debouncedLoad(char, count) {
		const wrappedResolve = (data) => {
			if (count === this._loadCounter) {
				var _this$_resolve;
				(_this$_resolve = this._resolve) === null || _this$_resolve === void 0 || _this$_resolve.call(this, data);
			}
		};
		const wrappedReject = (reason) => {
			if (count === this._loadCounter) {
				var _this$_reject;
				(_this$_reject = this._reject) === null || _this$_reject === void 0 || _this$_reject.call(this, reason);
			}
		};
		const returnedData = this._options.charDataLoader(char, wrappedResolve, wrappedReject);
		if (returnedData) if ("then" in returnedData) returnedData.then(wrappedResolve).catch(wrappedReject);
		else wrappedResolve(returnedData);
	}
	_setupLoadingPromise() {
		return new Promise((resolve, reject) => {
			...
			this._isLoading = false;
			(_this$_options$onLoad = (_this$_options = this._options).onLoadCharDataSuccess) === null || _this$_options$onLoad === void 0 || _this$_options$onLoad.call(_this$_options, data);
			return data;
		}).catch((reason) => {
			this._isLoading = false;
			...
			const err = /* @__PURE__ */ new Error(`Failed to load char data for ${this._loadingChar}`);
			err.reason = reason;
			throw err;
		});
	}
...
var HanziWriter = class HanziWriter {
	...
	static loadCharacterData(character, options = {}) {
		const loadingManager = (() => {
			...
			if ((_loadingManager === null || _loadingManager === void 0 ? void 0 : _loadingManager._loadingChar) === character && _loadingOptions === options) return _loadingManager;
	...
	showCharacter(options = {}) {
		...
		return this._withData(() => {
			...
			return (_this$_renderState = this._renderState) === null || _this$_renderState === void 0 ? void 0 : _this$_renderState.run(showCharacter("main", this._character, typeof options.duration === "number" ? options.duration : this._options.strokeFadeDuration)).then((res) => {
				...
				(_options$onComplete = options.onComplete) === null || _options$onComplete === void 0 || _options$onComplete.call(options, res);
				return res;
			});
		});
	}
	hideCharacter(options = {}) {
		...
		return this._withData(() => {
			...
			return (_this$_renderState2 = this._renderState) === null || _this$_renderState2 === void 0 ? void 0 : _this$_renderState2.run(hideCharacter("main", this._character, typeof options.duration === "number" ? options.duration : this._options.strokeFadeDuration)).then((res) => {
				...
				(_options$onComplete2 = options.onComplete) === null || _options$onComplete2 === void 0 || _options$onComplete2.call(options, res);
				return res;
			});
		});
	}
	animateCharacter(options = {}) {
		...
		return this._withData(() => {
			...
			return (_this$_renderState3 = this._renderState) === null || _this$_renderState3 === void 0 ? void 0 : _this$_renderState3.run(animateCharacter("main", this._character, this._options.strokeFadeDuration, this._options.strokeAnimationSpeed, this._options.delayBetweenStrokes)).then((res) => {
				...
				(_options$onComplete3 = options.onComplete) === null || _options$onComplete3 === void 0 || _options$onComplete3.call(options, res);
				return res;
			});
		});
	}
	animateStroke(strokeNum, options = {}) {
		...
		return this._withData(() => {
			...
			return (_this$_renderState4 = this._renderState) === null || _this$_renderState4 === void 0 ? void 0 : _this$_renderState4.run(animateSingleStroke("main", this._character, fixIndex(strokeNum, this._character.strokes.length), this._options.strokeAnimationSpeed)).then((res) => {
				...
				(_options$onComplete4 = options.onComplete) === null || _options$onComplete4 === void 0 || _options$onComplete4.call(options, res);
				return res;
			});
		});
	}
	highlightStroke(strokeNum, options = {}) {
		const promise = () => {
			if (!this._character || !this._renderState) return;
			return this._renderState.run(highlightStroke(selectIndex(this._character.strokes, strokeNum), colorStringToVals(this._options.highlightColor), this._options.strokeHighlightSpeed)).then((res) => {
				...
				(_options$onComplete5 = options.onComplete) === null || _options$onComplete5 === void 0 || _options$onComplete5.call(options, res);
				return res;
			});
		};
	...
	async loopCharacterAnimation() {
		this.cancelQuiz();
		return this._withData(() => this._renderState.run(animateCharacterLoop("main", this._character, this._options.strokeFadeDuration, this._options.strokeAnimationSpeed, this._options.delayBetweenStrokes, this._options.delayBetweenLoops), { loop: true }));
	}
	pauseAnimation() {
		return this._withData(() => {
	...
	showOutline(options = {}) {
		...
		return this._withData(() => {
			...
			return (_this$_renderState7 = this._renderState) === null || _this$_renderState7 === void 0 ? void 0 : _this$_renderState7.run(showCharacter("outline", this._character, typeof options.duration === "number" ? options.duration : this._options.strokeFadeDuration)).then((res) => {
				...
				(_options$onComplete6 = options.onComplete) === null || _options$onComplete6 === void 0 || _options$onComplete6.call(options, res);
				return res;
			});
		});
	}
	hideOutline(options = {}) {
		...
		return this._withData(() => {
			...
			return (_this$_renderState8 = this._renderState) === null || _this$_renderState8 === void 0 ? void 0 : _this$_renderState8.run(hideCharacter("outline", this._character, typeof options.duration === "number" ? options.duration : this._options.strokeFadeDuration)).then((res) => {
				...
				(_options$onComplete7 = options.onComplete) === null || _options$onComplete7 === void 0 || _options$onComplete7.call(options, res);
				return res;
			});
		});
	}
	...
	updateDimensions({ width, height, padding }) {
		...
		if (this._character && this._renderState && this._hanziWriterRenderer && this._positioner) {
			...
			if (this._quiz) this._quiz.setPositioner(this._positioner);
		}
	}
	updateColor(colorName, colorVal, options = {}) {
		var _options$duration;
		let mutations = [];
		const mappedColor = colorStringToVals((() => {
			if (colorName === "radicalColor" && !colorVal) return this._options.strokeColor;
			return colorVal;
		})());
		...
		if (colorName === "radicalColor" && !colorVal) mutations = mutations.concat(updateColor(colorName, null, 0));
		return this._withData(() => {
			...
			return (_this$_renderState9 = this._renderState) === null || _this$_renderState9 === void 0 ? void 0 : _this$_renderState9.run(mutations).then((res) => {
				...
				(_options$onComplete8 = options.onComplete) === null || _options$onComplete8 === void 0 || _options$onComplete8.call(options, res);
				return res;
			});
		});
	}
	...
	skipQuizStroke() {
		if (this._quiz) this._quiz.nextStroke();
	}
	cancelQuiz() {
		if (this._quiz) {
			this._quiz.cancel();
	...
	setCharacter(char) {
		...
		this._withDataPromise = this._loadingManager.loadCharData(char).then((pathStrings) => {
			if (!pathStrings || this._loadingManager.loadingFailed) return;
	...
	_assignOptions(options) {
		...
		if (options.strokeAnimationDuration && !options.strokeAnimationSpeed) mergedOptions.strokeAnimationSpeed = 500 / options.strokeAnimationDuration;
		if (options.strokeHighlightDuration && !options.strokeHighlightSpeed) mergedOptions.strokeHighlightSpeed = 500 / mergedOptions.strokeHighlightDuration;
		if (!options.highlightCompleteColor) mergedOptions.highlightCompleteColor = mergedOptions.highlightColor;
	...
	_fillWidthAndHeight(options) {
		...
		if (filledOpts.width && !filledOpts.height) filledOpts.height = filledOpts.width;
		else if (filledOpts.height && !filledOpts.width) filledOpts.width = filledOpts.height;
	...
	_withData(func) {
		...
		if (this._withDataPromise) return this._withDataPromise.then(() => {
			if (!this._loadingManager.loadingFailed) return func();
		});
	...
	_setupListeners() {
		...
		this.target.addPointerEndListener(() => {
			var _this$_quiz;
			(_this$_quiz = this._quiz) === null || _this$_quiz === void 0 || _this$_quiz.endUserStroke();
		});
	}
};
...
//#endregion
export { HanziWriter as default };

//# sourceMappingURL=hanzi-writer.js.map